...
| AUTH-03 | Chặn dò mật khẩu — hai lớp | Tài khoản hoạt động | a) Nhập sai 5 lần liên tiếp từ cùng một máy, lần 6 nhập **đúng** b) Nhập sai 10 lần từ nhiều IP khác nhau rồi nhập đúng | a) Lần 6 bị chặn 429 `TOO_MANY_ATTEMPTS` "chờ khoảng N phút" b) Bị chặn 401 `ACCOUNT_LOCKED`, UI "Thử lại sau 15 phút hoặc liên hệ BTC" | (email, IP): 5 lần sai / 15 phút → 429. Tài khoản: 10 lần sai → khoá 15 phút. Đăng nhập đúng, BTC gỡ khoá hoặc BTC đặt lại mật khẩu đều xoá cả hai bộ đếm | P1 | Đã sửa (task 1) | docs/09 §5.1 |
...
| EMP-13 | Tự huỷ sau hạn, trước khi công bố | Kỳ `registration_closed` hoặc `allocation_processing`, đã quá hạn đăng ký, đã được xếp bay / xe / phòng / ghế Gala, làm Trưởng xe **và Trưởng nhóm** | Trang Đăng ký → khối "Huỷ tham gia" → Huỷ tham gia → nhập lý do; Trưởng nhóm cũ thử giữ ghế Gala | Huỷ ngay; vé bay, xe, phòng, ghế Gala được gỡ, vai trò Trưởng xe **và Trưởng nhóm** bị bỏ; cờ phí phạt bật; email CBNV + mọi BTC (kèm cảnh báo team mất Trưởng nhóm); dòng huỷ ở `/admin/cancellations` gắn badge "Trưởng nhóm"; người cũ giữ/xem/gán ghế Gala → 403 | Sau `registration_closes_at` → thuộc diện phí phạt theo quy định đã đồng ý; gỡ chỗ cùng transaction; BTC chỉ định Trưởng nhóm mới trên dashboard (BTC-76) | P1 | Đã sửa (task 5 + fix Trưởng nhóm) | Trước đây không có nút huỷ sau khi đóng đăng ký, và huỷ không gỡ chỗ đã xếp ("ghế ma" làm chuyến bay/phòng báo đầy sai); Trưởng nhóm huỷ xong vẫn đổi được ghế Gala của team |
...
| EMP-15 | Đăng ký lại sau khi huỷ | Đã huỷ, kỳ chưa công bố (kể cả `registration_closed` / `allocation_processing`) | Mở trang Đăng ký → banner "Đăng ký lại" → điền lại form → gửi | Kích hoạt lại bản ghi cũ, xoá lý do huỷ và cờ phí phạt; chỗ bay/xe/phòng/ghế cũ không tự giữ lại; My Journey có nút "Đăng ký lại"; nếu BTC đã đóng đăng ký → BTC nhận email "đăng ký lại" (BTC-78) | Tới trước khi công bố; chức Trưởng nhóm / Trưởng xe cũ không tự khôi phục | P2 | Đã sửa (mở rộng tới trước công bố) | Trước đây chỉ đăng ký lại được khi còn mở đăng ký |
| EMP-15b | Không đăng ký lại sau công bố | Đã huỷ, kỳ `information_published` trở đi | Mở trang Đăng ký; gọi API gửi đăng ký | Thẻ "Bạn đã huỷ đăng ký" + hướng dẫn liên hệ BTC, không có form; API 409 `REGISTRATION_CANCELLED_FINAL` | Chặn cứng — muốn đi lại phải qua BTC xử lý ngoài hệ thống | P1 | Mới | |
...
| TL-14 | Trưởng xe xem hành khách | BTC gán người này làm Trưởng xe XE-01 | a) Mở My Journey b) Bấm "Danh sách hành khách" trên thẻ xe c) Gọi thẳng `GET /buses/{XE-02}/passengers` | a) Thẻ xe mình đi có huy hiệu "Bạn"; xe phụ trách mà không đi hiện ở khối "Xe bạn phụ trách" b) Modal: tên, team, điểm đón, SĐT bấm gọi — không CCCD, không ngày sinh c) 403 | `led_buses` trong `GET /journey/me` (rỗng khi chưa công bố); Trưởng xe chỉ xem xe mình (docs/09 §4) | P2 | Đã sửa (task 2) | docs/04 §9 |
...
| TL-16 | Trưởng nhóm huỷ đăng ký mất quyền ngay | Trưởng nhóm đang đăng nhập, đã được xếp ghế Gala cho team | Tự huỷ (trước công bố) hoặc được BTC duyệt huỷ; rồi thử giữ ghế, nhả ghế, xem thành viên, gán người, bấm "Xếp ngẫu nhiên" | Mọi thao tác Gala → 403; nhãn vai trò trở về "CBNV"; team không còn Trưởng nhóm cho tới khi BTC chỉ định | Chức gỡ cùng transaction huỷ; gate Gala từ chối người đã `cancelled` kể cả khi sót chức | P1 | Đã sửa (task 5 + fix Trưởng nhóm) | Trước đây Trưởng nhóm huỷ xong vẫn đổi được ghế Gala của cả team |
...
| BTC-02 | 4 chỉ số | – | So số trên thẻ với danh sách CBNV / đăng ký | Tổng nhân sự (mọi tài khoản đang hoạt động, gồm BTC/Super Admin — họ cũng là đối tượng đăng ký, tự đăng ký ở menu "Tham gia của tôi"); Đã phản hồi = đã gửi + đã huỷ (kèm %); Xác nhận tham gia (kèm không đi / huỷ); Chưa phản hồi — bấm vào mở `/admin/users?registration=none` | Tổng các dòng team = số tổng | P1 | Khớp | |
| BTC-03 | Việc cần làm theo giai đoạn | Kỳ `registration_open` | Xem thẻ "Việc cần làm"; chuyển kỳ sang `registration_closed` rồi xem lại | Đang mở đăng ký: chỉ "nhân sự chưa phản hồi", "Bổ sung giấy tờ", "Gửi lại email lỗi"; từ khi đóng đăng ký: thêm việc thiếu ghế / xếp bay / xe / phòng; mỗi dòng có link hoặc nút gửi nhắc đúng chỗ; việc đã xong thu gọn "Đã xong x/y mục" | Đã công bố mà còn thiếu → mức Khẩn (đỏ) | P1 | Đã sửa (B25) | Gộp 4 thẻ cũ (checklist, nhắc email, cảnh báo giấy tờ, email) vốn lặp lại cùng con số |
...
| BTC-39 | Gán Trưởng xe | – | Chọn CBNV làm Trưởng xe (hoặc nhập tên + SĐT) | Hiện trên thẻ xe và thẻ Xe trong My Journey của hành khách (gọi được); người được gán thấy xe đó trong My Journey của mình | – | P2 | Khớp | Xem TL-14 |
...
| BTC-68 | Thông báo cho CBNV | Kỳ đã công bố, đổi giờ bay | Mở `/admin/announcements` → Soạn → chọn đối tượng chuyến bay → Đăng + tick gửi email | Theo docs/01 §3: BTC "điều chỉnh + gửi thông báo thay đổi"; đúng nhóm nhận thấy trong My Journey; thư `announcement_notice` trong nhật ký email | Nháp chỉ BTC thấy; đối tượng sai kỳ → 422; gỡ/xoá có audit | P2 | Khớp | Đã làm (task 3): xem EMP-23 |
...
| BTC-76 | Chỉ định Trưởng nhóm trên dashboard | Trưởng nhóm team X vừa tự huỷ; team còn người đang tham gia | Dashboard → "Việc cần làm" hiện "Team X chưa có Trưởng nhóm đang tham gia" → Chỉ định Trưởng nhóm (hoặc cột "Trưởng nhóm" trong bảng team → Chỉ định); thử chọn người ngoài team / người chưa đăng ký qua API | Hộp thoại chỉ liệt kê thành viên team đang tham gia; chỉ định xong: việc cần làm biến mất, người mới thành "Trưởng nhóm" và chọn ghế Gala được ngay; API: `LEADER_NOT_IN_TEAM`, `LEADER_NOT_PARTICIPATING`, `LEADER_IS_ORGANIZER`; CBNV gọi → 403 | Vai trò đi theo chức; audit `team.leader_changed`; đã công bố thì việc này ở mức Khẩn | P1 | Khớp | Trước đây email bảo "gán ở Master data" nhưng không có màn hình nào để gán |
| BTC-77 | Duyệt huỷ Trưởng nhóm kèm người thay | Trưởng nhóm gửi yêu cầu huỷ sau công bố | Mở Duyệt → khối "Người này đang là Trưởng nhóm" → chọn Trưởng nhóm mới → Duyệt huỷ; lần khác gửi người thay không hợp lệ qua API | Huỷ + gỡ chỗ + đổi Trưởng nhóm trong **một** thao tác; người thay không hợp lệ → lỗi và **không** huỷ gì (yêu cầu vẫn chờ, Trưởng nhóm cũ giữ nguyên); gửi người thay cho người không phải Trưởng nhóm → `NOT_TEAM_LEADER`; "Huỷ thay CBNV" cũng có khối chọn người thay | Cùng transaction | P1 | Khớp | |
| BTC-78 | Báo BTC khi có người đăng ký lại | Kỳ `allocation_processing`, CBNV đã tự huỷ | CBNV đăng ký lại; BTC xem email, dashboard, `/admin/cancellations` | Mọi BTC nhận email "đăng ký tham gia lại sau khi huỷ"; "Việc cần làm" hiện "N CBNV đăng ký lại sau khi huỷ"; dòng huỷ có badge "Đã đăng ký lại"; đăng ký lại lúc còn mở đăng ký thì **không** gửi thư này | Chỗ cũ không tự khôi phục — BTC xếp lại | P2 | Khớp | |
| BTC-79 | Tra cứu lộ trình một người | Kỳ `allocation_processing` (chưa công bố), một người đã được xếp bay/xe/phòng/ghế | Mở `/admin/people` → gõ tên không dấu → chọn người | Hiện đúng chuyến đi/về (kèm ghế), xe từng chặng, phòng (kèm tầng), ghế Gala; chặng chưa xếp ghi "chưa có xe" chứ không bỏ trống; mỗi mục bấm được để nhảy sang màn hình tương ứng | Không chặn theo `published` (khác `/journey/{user_id}`); không CCCD/ngày sinh | P1 | Khớp | |
| BTC-80 | Tô đỏ chỗ của người tra cứu | Đang tra cứu một người ở `/admin/flights`, `/admin/buses`, `/admin/rooms`, `/admin/gala`, `/admin/flights/board` | Nhìn màn hình sau khi chọn người | Dòng/thẻ/ghế của họ viền đỏ + tự cuộn tới; tab chặng/khách sạn/chiều bay tự chuyển sang đúng chỗ; thẻ team gập trên bảng bay tự bung; phòng bị bộ lọc ẩn thì có banner + nút bỏ lọc | `?person=` trên URL nên F5 không mất; tắt tra cứu là hết tô | P1 | Khớp – test tay UI | |
| BTC-81 | Tra cứu người chưa xếp / đã huỷ | Một người chưa có xe chặng 2; một người đã huỷ đăng ký | Tra cứu từng người | Người chưa xếp: chặng đó ghi "chưa có xe"; người đã huỷ: báo trạng thái huỷ, không hiện lỗi | Khung response đầy đủ kể cả khi không tham gia | P2 | Khớp | |
...
| SEC-10 | Brute-force đăng nhập theo IP | – | a) Từ 1 IP thử nhiều email khác nhau, mỗi email 4 lần sai b) Gửi kèm header `X-Forwarded-For` giả, đổi giá trị mỗi lần | a) Tới lần sai thứ 20 của IP đó thì mọi email đều bị 429, kể cả email chưa từng thử; IP khác không bị ảnh hưởng b) Header giả không đổi được gì — `login_attempts` vẫn ghi IP thật | 20 lần sai / 15 phút / IP. `get_client_ip` ưu tiên `X-Real-IP`, rồi phần tử **cuối** của `X-Forwarded-For`; nginx đặt cả hai từ `$remote_addr` | P2 | Đã sửa (task 1) | docs/09 §5.1, §5.2 |
...
| NFR-08 | Không trắng trang | – | `npm run check:render` | 139 kịch bản OK | Bẫy icon lucide trùng tên API JS | P1 | Khớp | |
...
|---|---|---|---|---|
| 1 | Đã sửa (task 1) | Khoá đăng nhập chỉ theo tài khoản, chưa theo IP như docs/09 → dò mật khẩu rải nhiều email được; ai biết email có thể cố tình khoá tài khoản người khác. `X-Forwarded-For` do client tự gửi cũng làm IP trong audit log không đáng tin | AUTH-03, SEC-10 | Bảng `login_attempts` + `login_guard`: 5 lần sai / 15 phút / (email, IP) và 20 lần sai / 15 phút / IP → 429, chặn trước khi so mật khẩu; khoá theo tài khoản giữ nguyên nhưng nâng ngưỡng lên 10; `get_client_ip` lấy IP do proxy nối vào, nginx đặt `X-Forwarded-For $remote_addr` |
| 2 | Đã sửa (task 2) | Trưởng xe có quyền API xem hành khách xe mình nhưng không có màn hình | TL-14 | `led_buses` trong `GET /journey/me` + nút "Danh sách hành khách" trên thẻ Xe, thêm khối "Xe bạn phụ trách" cho xe họ không tự đi; modal chỉ đọc (tên, team, điểm đón, SĐT bấm gọi) |
| 3 | Đã sửa (task 3) | BTC không tạo được thông báo; panel thông báo chỉ có dữ liệu seed | EMP-23, BTC-68 | API + màn hình tạo thông báo theo đối tượng (mọi người / team / chuyến bay / xe / cá nhân), có gửi email |
...
**Tổng số case:** 15 AUTH · 11 CHAT · 35 EMP · 16 TL · 81 BTC · 13 SA · 19 SEC/NFR = **190 case**.
